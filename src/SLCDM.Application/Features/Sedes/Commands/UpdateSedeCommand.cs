...
public sealed class UpdateSedeCommandValidator : AbstractValidator<UpdateSedeCommand>
{
    public UpdateSedeCommandValidator(IApplicationDbContext db)
    {
        ...
        RuleFor(x => x.Nombre)
            ...
            .MustAsync(async (cmd, nombre, ct) =>
            {
                ...
                return !await db.Sedes.IgnoreQueryFilters()
                    .AnyAsync(s => s.IdEmpresa == cmd.IdEmpresa
                        && s.Nombre.ToLower() == normalized
                        && s.Id != cmd.Id, ct);
            })
        ...
        RuleFor(x => x.Direccion)
            ...
            .MustAsync(async (cmd, direccion, ct) =>
            {
                ...
                var normalized = direccion.Trim().ToLower();
                return !await db.Sedes.IgnoreQueryFilters()
                    .AnyAsync(
                        s => s.IdEmpresa == cmd.IdEmpresa
                             && s.Id != cmd.Id
                             && s.Direccion != null
                             && s.Direccion.ToLower() == normalized,
                        ct);
            })
            .WithMessage("Ya existe una sede con la misma direccion en esta empresa.")
